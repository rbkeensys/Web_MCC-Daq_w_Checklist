...
async function openVfdEditor(){
  ...
  function steppersPane(){
    ...
    stepConfigs.forEach(c=>{
      ...
      const holdRegInp = el('input',{type:'text',value:(c.standstill_current_reg||'Pr5.01'),style:'width:62px',
        title:'Drive parameter that Hold% writes to. Default Pr5.01 is a BEST GUESS -- verify against your DM556RS manual (or use #0xADDR).'});
      holdRegInp.oninput=()=>{c.standstill_current_reg = holdRegInp.value.trim()||'Pr5.01';};
    ...
    wrap.append(el('div',{style:'margin-top:8px'}, el('button',{className:'btn',onclick:()=>{
      stepConfigs.push({key:`stepper${stepConfigs.length+1}`,name:'New stepper',steps_per_rev:10000,
        ...
        standstill_current_pct:null,standstill_current_reg:'Pr5.01'});
      draw();
    }},'+ Add stepper')));
    return wrap;
  }
