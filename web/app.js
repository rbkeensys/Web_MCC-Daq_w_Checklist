...
async function openConfigForm(providedConfig = null, banner = null){
  ...
  // ---- Remote access (server 2.13.23): view-only Cloudflare identities ----
  const viewersInp = el('input', {type: 'text', value: (cfg.viewOnlyUsers || []).join(', '),
    style: 'width:340px', placeholder: 'email1@x.com, email2@y.com'});
  viewersInp.oninput = () => {
    cfg.viewOnlyUsers = viewersInp.value.split(',').map(x => x.trim().toLowerCase()).filter(Boolean);
  };
  const ctlInp = el('input', {type: 'text', value: (cfg.controlUsers || []).join(', '),
    style: 'width:340px', placeholder: 'empty = everyone authenticated may control'});
  ctlInp.oninput = () => {
    cfg.controlUsers = ctlInp.value.split(',').map(x => x.trim().toLowerCase()).filter(Boolean);
  };
  const remote = fieldset('Remote access', el('div', {}, [
    el('p', {style: 'font-size:12px;color:var(--muted);margin-bottom:8px'},
      'CONTROL emails: when this list has ANY entries, only those Cloudflare Access identities may control -- every other authenticated visitor is view-only automatically (set the Access policy to Everyone + one-time PIN for a public view-only dashboard). VIEW-ONLY emails: always watch-only regardless. Local (kiosk/LAN) connections are never restricted. Any browser can self-restrict with ?viewer=1 in the URL.'),
    tableForm([
      ['Control emails', ctlInp],
      ['View-only emails', viewersInp]
    ])
  ]));
