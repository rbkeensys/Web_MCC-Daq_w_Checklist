...
async function openConfigForm(providedConfig = null, banner = null){
  ...
  const remote = fieldset('Remote access', el('div', {}, [
    el('p', {style: 'font-size:12px;color:var(--muted);margin-bottom:8px'},
      'View-only accounts: Cloudflare Access emails that may WATCH but not control -- every control/edit API returns 403 and the UI hides its controls. Local (kiosk/LAN) connections are never restricted. Any browser can also self-restrict by adding ?viewer=1 to the URL.'),
    tableForm([['View-only emails', viewersInp]])
  ]));
