...
function hookLogButtons(){
  ...
  if (viewerBtn && !viewerBtn._wired){
    // LOCAL VIEWER (2.1.112): open the viewer on THIS computer (the one
    // that clicked). We POST the scales bundle (bundle_only -- the server
    // prepares but does not launch), then invoke the mccviewer: protocol;
    // the machine's registered handler (tools/register_viewer_protocol.bat,
    // once per computer) downloads the log and runs log_viewer.py locally.
    // "Viewer (rig screen)" in the Menu keeps the old on-server launch.
    window._launchViewer = async function (local) {
      // ORDER MATTERS (2.1.113): the mccviewer: navigation must fire
      // SYNCHRONOUSLY inside the click -- Chrome requires transient user
      // activation for external-protocol launches and SILENTLY drops the
      // navigation once the activation is spent on awaited fetches (russ:
      // F5/Ctrl-F5 many times, viewer never opens, no dialog). The bundle
      // posts in parallel below; the local launcher retries fetching it.
      if (local) {
        window.location.href = 'mccviewer:' + encodeURIComponent(window.location.origin);
        console.log('[Viewer] invoked mccviewer: handler -- if nothing opens, run tools/register_viewer_protocol.bat once on this computer and allow the browser prompt');
      }
      try{
        ...
        const r = await fetch('/api/log_viewer/launch', {
          ...
          body: JSON.stringify({ scales: _collectChartScales(), names, charted,
                                 colors: _collectChartColors(),
                                 bundle_only: !!local })
        });
        const j = await r.json();
        if (!j.ok) { alert('Viewer launch failed:\n' + (j.error || 'unknown error')); return; }
        // Success is otherwise silent — the viewer window appears on its own.
      }catch(e){
        alert('Viewer launch failed: ' + e.message);
      }
    };
    viewerBtn.addEventListener('click', ()=> window._launchViewer(true));
  ...
  // Reset Totals (2.1.109): archive-then-zero the machine lifetime odometers.
  // The server writes machineTotals_<date>_<time>.json BEFORE zeroing, so the
  // history is always reviewable; the reset is legitimate (guard baseline,
  // statics, and file move together -- no server stop, no restart).
  const resetTotalsBtn = document.getElementById('resetTotalsBtn');
  if (resetTotalsBtn && !resetTotalsBtn._wired){
    resetTotalsBtn.addEventListener('click', async ()=>{
      const kwh = state?.static_vars?.energyKWh, prodL = state?.static_vars?.energyProdL;
      const cur = (typeof kwh === 'number' && typeof prodL === 'number')
        ? '\nCurrent: ' + kwh.toFixed(3) + ' kWh, ' + prodL.toFixed(2) + ' L produced.' : '';
      if (!confirm('Reset the machine lifetime totals to zero?' + cur +
                   '\nThe current values are archived to a dated file first.')) return;
      try {
        const r = await fetch('/api/totals/reset', { method: 'POST' });
        const j = await r.json();
        if (j.ok) {
          alert('Totals reset.\nArchived to ' + j.archived +
                '\n(' + j.previous.energyKWh.toFixed(3) + ' kWh, ' +
                j.previous.energyProdL.toFixed(2) + ' L)');
        } else {
          alert(j.error || 'Reset failed.');
        }
      } catch (e) { alert('Reset failed: ' + e.message); }
    });
    resetTotalsBtn._wired = true;
  }
...
/* ---- SD/disk space guard (2.1.108) ------------------------------------
   Server broadcasts {type:'disk_alert', active, pct, free_gb} (1/min while
   active; also in the WS hello). A modal on EVERY connected computer
   forces a decision: download the closed log sessions then delete them,
   or delete oldest until below target. Whichever computer acts, space
   recovers and the next broadcast (active:false) removes the modal on
   all watchers. Snooze hides it locally for 30 min. */
function showDiskAlert(a) {
  let m = document.getElementById('diskAlertModal');
  if (!a.active) { if (m) m.remove(); return; }
  if (window._diskSnoozeUntil && Date.now() < window._diskSnoozeUntil) return;
  if (m) {
    const st = m.querySelector('#diskAlertStats');
    if (st) st.textContent = a.pct + '% full \u2014 ' + a.free_gb + ' GB free';
    return;
  }
  m = document.createElement('div');
  m.id = 'diskAlertModal';
  m.style.cssText = 'position:fixed;inset:0;z-index:11000;background:rgba(0,0,0,.55);display:flex;align-items:center;justify-content:center';
  const box = document.createElement('div');
  box.style.cssText = 'background:#1e2235;border:1px solid #ff6b6b;border-radius:10px;padding:18px 22px;max-width:480px;color:#e6e6e6;font:13px/1.5 system-ui;box-shadow:0 8px 32px rgba(0,0,0,.6)';
  box.innerHTML =
    '<h3 style="margin:0 0 6px;color:#ff6b6b">Storage nearly full</h3>' +
    '<div id="diskAlertStats" style="margin-bottom:10px">' + a.pct + '% full \u2014 ' + a.free_gb + ' GB free</div>' +
    '<div style="margin-bottom:12px;color:#a8b3cf">Logging fails when the card fills. Handle it from any computer \u2014 this popup clears everywhere once space is freed.</div>' +
    '<div style="display:flex;gap:8px;flex-wrap:wrap">' +
    '<button id="diskDlBtn" class="btn">\u2b07 Download logs\u2026</button>' +
    '<button id="diskPruneBtn" class="btn">\ud83d\uddd1 Delete oldest (to 85%)</button>' +
    '<button id="diskSnoozeBtn" class="btn">Snooze 30 min</button>' +
    '</div><div id="diskDlPanel" style="margin-top:12px;max-height:40vh;overflow-y:auto"></div>';
  m.append(box);
  document.body.append(m);
  box.querySelector('#diskPruneBtn').onclick = async () => {
    if (!confirm('Delete the OLDEST closed log sessions until the card is below 85%?')) return;
    try {
      const j = await (await fetch('/api/logs/prune', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}' })).json();
      alert(j.ok ? ('Deleted: ' + (j.deleted.join(', ') || 'nothing') + '\nNow ' + j.pct + '% full.') : (j.error || 'Prune failed'));
    } catch (e) { alert('Prune failed: ' + e.message); }
  };
  box.querySelector('#diskSnoozeBtn').onclick = () => {
    window._diskSnoozeUntil = Date.now() + 30 * 60 * 1000;
    m.remove();
  };
  box.querySelector('#diskDlBtn').onclick = async () => {
    const panel = box.querySelector('#diskDlPanel');
    let j;
    try { j = await (await fetch('/api/logs/sessions')).json(); }
    catch (e) { panel.textContent = 'Failed to list sessions: ' + e.message; return; }
    if (!j.sessions || !j.sessions.length) {
      panel.textContent = 'No closed sessions to download (only the active log remains).';
      return;
    }
    panel.innerHTML = '<div style="color:#a8b3cf;margin-bottom:6px">Click each file to download; when your browser shows them complete, press delete.</div>';
    j.sessions.forEach(sess => {
      sess.files.forEach(f => {
        const aEl = document.createElement('a');
        aEl.href = '/api/logs/download/' + encodeURIComponent(sess.name) + '/' + encodeURIComponent(f);
        aEl.download = sess.name + '_' + f;
        aEl.textContent = '\u2b07 ' + sess.name + '/' + f + '  (' + (sess.bytes / 1e6).toFixed(0) + ' MB session)';
        aEl.style.cssText = 'display:block;color:#7fb3ff;margin:2px 0';
        panel.append(aEl);
      });
    });
    const done = document.createElement('button');
    done.className = 'btn';
    done.textContent = 'Downloads finished \u2014 delete these logs';
    done.style.marginTop = '8px';
    done.onclick = async () => {
      if (!confirm('Delete ALL closed log sessions from the server? Make sure your downloads completed first.')) return;
      try {
        const r = await (await fetch('/api/logs/prune', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{"all":true}' })).json();
        alert(r.ok ? ('Deleted: ' + (r.deleted.join(', ') || 'nothing') + '\nNow ' + r.pct + '% full.') : (r.error || 'Delete failed'));
...
const FAULT_MSGS = {
  ...
  condFlowErr: 'Condensate Flowmeter or Pump FAILURE\n\n'
    + 'The condensate flow meter read EXACTLY zero for 30 continuous\n'
    + 'seconds while running -- the meter or its pump (stepper driver?)\n'
    + 'is dead. The run has been TRIPPED and halted.\n\n'
    + 'Check the stepper driver, pump head, and meter wiring, then\n'
    + 'stop/restart the run to clear.',
...
/* ---- top-bar dropdown menus (2.1.110) ---------------------------------
   The bar had grown past the display width; only the frequently-used
   buttons stay permanent (per russ 9/11). The rest are MOVED (DOM move --
   listeners survive) into two dropdown panels: "Layout" (save/load/shared)
   and "Menu" (everything else, separated by functional group). Entries
   whose button stays permanent get a synthetic row that clicks the real
   button, so everything is reachable from the menu too. */
function initTopbarMenus() {
  ...
  function synth(label, targetId, extra) {
    const b = document.createElement('button');
    b.type = 'button';
    b.className = 'btn';
    b.textContent = label;
    b.onclick = () => { closeAll(); if (extra) extra(); else grab(targetId)?.click(); };
    return b;
  }
  function sep() {
  ...
  const panels = [];
  function closeAll() { panels.forEach(dd => dd.classList.remove('open')); }
  function wireToggle(btn, dd) {
    panels.push(dd);
    btn.onclick = (e) => {
      e.stopPropagation();
      ...
      if (!was) {
        ...
        dd.style.right = Math.max(4, window.innerWidth - r.right) + 'px';
        dd.style.left = 'auto';
        dd.classList.add('open');
      }
    };
  }
  // close on any outside click; a moved button's own click also closes
  document.addEventListener('pointerdown', (ev) => {
    if (!panels.some(dd => dd.contains(ev.target)) &&
        ev.target !== grab('tbLayoutBtn') && ev.target !== grab('tbMoreBtn')) closeAll();
  }, true);
  document.addEventListener('click', (ev) => {
    if (panels.some(dd => dd.contains(ev.target)) && ev.target.classList.contains('btn') &&
        ev.target.id !== 'sharedLayouts') setTimeout(closeAll, 50);
  }, true);

  // ---- "Layout" panel: the real save/load/shared buttons move in ----
  const ddLayout = makePanel('tbLayoutDD');
  ['saveLayout', 'loadLayout', 'sharedLayouts'].forEach(id => {
    const b = grab(id); if (b) ddLayout.append(b);
  });
  wireToggle(grab('tbLayoutBtn'), ddLayout);

  // ---- "Menu" panel: functional groups (russ 9/11 spec) ----
  const ddMore = makePanel('tbMoreDD');
  const move = id => { const b = grab(id); if (b) ddMore.append(b); };
  // 1: config / calibrate / zero
  ddMore.append(synth('Config', 'editConfig'));
  move('calibrateAI');
  ddMore.append(synth('\u26A1 Zero', 'zeroAI'));
  ddMore.append(sep());
  // 2: expression help + expressions
  move('exprHelp');
  ddMore.append(synth('Expressions', 'editExpr'));
  ddMore.append(sep());
  // 3: sim
  move('editSim');
  ddMore.append(sep());
  // 4: drives / scales / pid / script
  move('editVFD'); move('editScales'); move('editPID'); move('editScript');
  ddMore.append(sep());
  // 5: logs
  move('viewerBtn');
  ddMore.append(synth('\U0001F5A5 Viewer (rig screen)', null, () => window._launchViewer?.(false)));
  ddMore.append(synth('Start New Log', 'closeLogBtn'));
  move('openLogBtn'); move('mergeChkBtn');
  ddMore.append(sep());
  // 6: layout (duplicates -- open the Layout panel for the shared list so
  //    its submenu anchors to a VISIBLE button)
  ddMore.append(synth('Load Layout', 'loadLayout'));
  ddMore.append(synth('Save Layout', 'saveLayout'));
  ddMore.append(synth('Layouts \u25BE', null, () => {
    grab('tbLayoutBtn').click();
    setTimeout(() => grab('sharedLayouts')?.click(), 60);
  }));
  ddMore.append(sep());
  // 7: totals
  move('resetTotalsBtn');
  wireToggle(grab('tbMoreBtn'), ddMore);
}

document.addEventListener('DOMContentLoaded', () => {
  try { initTopbarMenus(); } catch (e) { console.warn('[TopbarMenus] init failed:', e); }
  if (IS_POPOUT) {
    initPopout();
    return;
  }
  ...
  _loadUiEditMode();
  ensureStarterPage();
  const _hadLocal = loadLayoutFromLocal();    // THIS browser's last-saved layout, or starter page
  if (!_hadLocal) {
    // SERVER DEFAULT LAYOUT (russ 7/27): a brand-new browser (phone through the
    // tunnel) applies the rig's default automatically -- no file picker on a
    // tiny screen. Set it from any full browser: Save Layout -> default prompt.
    fetch('/api/layout/default').then(r => r.json()).then(obj => {
      if (obj && obj.pages) {
        applyLayoutObject(obj);
        _persistLayoutLocal(obj);
        renderPage();
        console.log('[Layout] applied SERVER DEFAULT layout');
      }
    }).catch(() => {});
  }
  _wireMultiSelect();
...
function wireUI(){
  ...
  $('#sharedLayouts')?.addEventListener('click', openLayoutsMenu);
...
function connect(){
  ...
  ws.onmessage=(ev)=>{
    ...
    if (msg.type === 'disk_alert') showDiskAlert(msg);   // SD space guard
    if (msg.type === 'hello') {
      ...
      if (typeof msg.readonly !== 'undefined') { window.MCC_READONLY = !!msg.readonly; _applyViewerMode(); }
      if (msg.disk_alert) showDiskAlert(Object.assign({type:'disk_alert'}, msg.disk_alert));   // late joiners see an active alert
      updateDOButtons();
    }
  };
  updateConnectBtn();
...
/* ---- KIOSK DISPLAY (russ 7/26): topbar hide + fit-to-window zoom ----
   Fit uses CSS zoom on #canvas (Chromium-native): whole dashboard scales to
   the visible area, scrollbars gone. Released while the layout is UNLOCKED so
   drag/resize/snap math stays 1:1; re-applies on lock. Both states persist. */
const TB_HIDE_KEY = 'mcc.topbarHidden';
const FIT_KEY = 'mcc.fitZoom';
function applyFitZoom() {
  const c = document.getElementById('canvas');
  if (!c) return;
  let want = false;
  try { want = localStorage.getItem(FIT_KEY) === '1'; } catch {}
  const btn = document.getElementById('fitBtn');
  if (btn) btn.style.background = want ? '#2f6feb' : '';
  c.style.zoom = '';
  if (!want || isUiEditMode()) return;
  const availW = c.clientWidth, availH = c.clientHeight;
  const needW = c.scrollWidth, needH = c.scrollHeight;
  if (needW <= 0 || needH <= 0) return;
  const z = Math.min(availW / needW, availH / needH, 1);
  if (z < 0.999) c.style.zoom = String(Math.max(0.25, z));
}
function _applyTopbarHidden(hidden) {
  const tb = document.querySelector('header.topbar');
  const show = document.getElementById('tbShowBtn');
  if (tb) tb.style.display = hidden ? 'none' : '';
  if (show) show.style.display = hidden ? 'block' : 'none';
  try { localStorage.setItem(TB_HIDE_KEY, hidden ? '1' : '0'); } catch {}
  setTimeout(applyFitZoom, 50);   // freed height changes the fit
}
/* VIEW-ONLY mode (server 2.13.23): the server flags Cloudflare-Access
   identities listed in config viewOnlyUsers (enforced server-side with 403s);
   any browser can also self-restrict with ?viewer=1. The UI hides every
   control surface -- clean glanceable dashboard for a phone. */
function _isViewer() {
  try { if (new URLSearchParams(location.search).has('viewer')) return true; } catch {}
  return !!window.MCC_READONLY;
}
function _applyViewerMode() {
  document.body.classList.toggle('viewer', _isViewer());
}

function _wireKioskDisplay() {
  document.getElementById('fitBtn')?.addEventListener('click', () => {
    try { localStorage.setItem(FIT_KEY, localStorage.getItem(FIT_KEY) === '1' ? '0' : '1'); } catch {}
    applyFitZoom();
  });
  document.getElementById('tbHideBtn')?.addEventListener('click', () => _applyTopbarHidden(true));
  document.getElementById('tbShowBtn')?.addEventListener('click', () => _applyTopbarHidden(false));
  window.addEventListener('resize', () => { clearTimeout(window._fitT); window._fitT = setTimeout(applyFitZoom, 150); });
  const c = document.getElementById('canvas');
  if (c) new MutationObserver(() => {
    clearTimeout(window._fitT); window._fitT = setTimeout(applyFitZoom, 120);
  }).observe(c, { childList: true });
  // TAP RIPPLE: a ring flashes where the browser ACTUALLY receives each
  // tap/click -- ground truth for the touch panel (kiosk taps land off-target
  // and the compositor cursor is unreliable there).
  ...
  // CURSOR IDLE AUTO-HIDE (kiosk touchscreen): the pointer (touch taps move
  // it via labwc mouseEmulation) vanishes after 3s idle, reappears on any
  // movement/tap. Class-based so it overrides per-element cursor styles.
  let _curT = null;
  const _showCursor = () => {
    document.body.classList.remove('cursor-idle');
    clearTimeout(_curT);
    _curT = setTimeout(() => document.body.classList.add('cursor-idle'), 3000);
  };
  ['mousemove', 'mousedown', 'touchstart'].forEach(ev =>
    window.addEventListener(ev, _showCursor, { passive: true }));
  _showCursor();
  try { if (localStorage.getItem(TB_HIDE_KEY) === '1') _applyTopbarHidden(true); } catch {}
  _applyViewerMode();
  setTimeout(applyFitZoom, 300);
}

function _wireUiEditToggle() {
  const btn = document.getElementById('uiEditToggle');
  if (!btn) return;
  btn.addEventListener('click', () => { _setUiEditMode(!isUiEditMode()); applyFitZoom(); });
}
...
/* CSV columns that appear as a series on ANY chart, in encounter order.
   ...
   config doesn't name.                                                     */
/* Chart series colors keyed by CSV column — the viewer inherits the exact
 * colors from the app's charts (first chart wins on duplicates). */
function _collectChartColors() {
  const out = {};
  const add = (c, color) => { if (c && color && out[c] === undefined) out[c] = color; };
  try {
        ...
        for (const s of (w.opts?.series || [])) {
          ...
          switch (s.kind) {
            ...
            case 'expr': case 'math': case 'scale':
              add(`${s.kind}${s.index|0}`, col); break;
            case 'pid':
              add(`pid${s.index|0}`, col); add(`pid${s.index|0}_out`, col); break;
            case 'button':
              add(`bvar_${s.index}`, col); break;
            case 'static': case 'global':
              add(`gvar_${s.index}`, col); add(`sv_${s.index}`, col); break;
          }
        }
      }
    }
  } catch (e) { console.warn('[Viewer] color collection failed:', e); }
  return out;
}
...
/* Snapshot the CURRENT layout as a plain object (pages + grid + checklist).
 * Shared by Save Layout (file download) and the shared-layout Publish. */
function _captureLayoutObject() {
...
function saveLayoutToFile() {
  const layout = _captureLayoutObject();
  ...
  a.click();
  // SERVER DEFAULT (russ 7/27): one tap here makes this layout what every
  // brand-new device (phone via Cloudflare) opens with automatically.
  if (!_isViewer() && confirm('Also set this layout as the SERVER DEFAULT that new devices load automatically?')) {
    fetch('/api/layout/default', {method: 'PUT', headers: {'Content-Type': 'application/json'},
      body: JSON.stringify(layout)})
      .then(r => r.json()).then(d => { if (!d || d.ok === false) alert('Server default save failed'); })
      .catch(e => alert('Server default save failed: ' + e.message));
  }
}

/* ===== SHARED LAYOUT LIBRARY (2.1.105, russ 7/29) =====
 * Named layouts stored ON THE SERVER (config/layouts/) so any visitor --
 * full control or view-only -- can browse and apply them without shipping
 * files around. Applying one is purely local (it becomes this browser's
 * last-used, same as Load Layout); Publish/Delete are control-only (the
 * server 403s mutating verbs for view-only identities). The last-used
 * auto-restore and the first-time server default are untouched. */
let _layoutsMenuCleanup = null;   // unregisters the click-away handler
function _closeLayoutsMenu() {
  document.getElementById('layoutsMenu')?.remove();
  // ALWAYS unhook the click-away listener (2.1.106 fix): closing via a menu
  // pick left it registered, and on the NEXT menu open the stale handler saw
  // the pick's pointerdown as "outside my (long-gone) menu" and closed the
  // fresh menu before the click could land -- every second layout pick died
  // until an F5 cleared the handlers.
  if (_layoutsMenuCleanup) { _layoutsMenuCleanup(); _layoutsMenuCleanup = null; }
}
function _loadSharedLayout(name) {
  fetch('/api/layouts/' + encodeURIComponent(name), {cache: 'no-store'})
    .then(r => r.json()).then(obj => {
      if (!obj || !obj.pages) {
        alert('Layout load failed' + (obj && obj.error ? ': ' + obj.error : ''));
        return;
      }
      applyLayoutObject(obj);
      _persistLayoutLocal(obj);   // becomes this browser's last-used, like any load
      renderPage();
      console.log('[Layout] applied shared layout', name);
    }).catch(e => alert('Layout load failed: ' + e.message));
}
function openLayoutsMenu() {
  if (document.getElementById('layoutsMenu')) { _closeLayoutsMenu(); return; }
  const btn = $('#sharedLayouts');
  fetch('/api/layouts', {cache: 'no-store'}).then(r => r.json()).then(d => {
    const names = (d && d.names) || [];
    const m = el('div', {id: 'layoutsMenu', className: 'layouts-menu'});
    if (!names.length) m.append(el('div', {className: 'layouts-empty'}, 'No shared layouts yet'));
    for (const n of names) {
      const row = el('div', {className: 'layouts-row'});
      const load = el('button', {className: 'layouts-load', title: 'Apply this layout'}, n);
      load.onclick = () => { _closeLayoutsMenu(); _loadSharedLayout(n); };
      row.append(load);
      if (!_isViewer()) {
        const del = el('button', {className: 'layouts-del', title: 'Delete from the server'}, '✕');
        del.onclick = (ev) => {
          ev.stopPropagation();
          if (!confirm('Delete shared layout "' + n + '" from the server?')) return;
          fetch('/api/layouts/' + encodeURIComponent(n), {method: 'DELETE'})
            .then(r => r.json()).then(x => {
              if (!x || x.ok === false) alert('Delete failed' + (x && x.error ? ': ' + x.error : ''));
              _closeLayoutsMenu(); openLayoutsMenu();   // refresh the list
            }).catch(e => alert('Delete failed: ' + e.message));
        };
        row.append(del);
      }
    ...
    if (!_isViewer()) {
      const pub = el('button', {className: 'layouts-publish'}, '➕ Publish current…');
      pub.onclick = () => {
        _closeLayoutsMenu();
        const name = prompt('Publish the CURRENT layout to the server as:');
        if (!name || !name.trim()) return;
        fetch('/api/layouts/' + encodeURIComponent(name.trim()), {
          method: 'PUT', headers: {'Content-Type': 'application/json'},
          body: JSON.stringify(_captureLayoutObject())})
          .then(r => r.json()).then(x => {
            if (!x || x.ok === false) alert('Publish failed' + (x && x.error ? ': ' + x.error : ''));
    ...
    document.body.append(m);
    // Click-away closes (deferred so the opening click doesn't self-close).
    // The handler's removal lives in _closeLayoutsMenu so EVERY close path
    // (pick, publish, toggle, click-away) unhooks it -- see 2.1.106 note.
    setTimeout(() => {
      if (document.getElementById('layoutsMenu') !== m) return;  // already closed/replaced
      const h = (ev) => {
        if (!m.contains(ev.target) && ev.target !== btn) _closeLayoutsMenu();
      };
      document.addEventListener('pointerdown', h);
      _layoutsMenuCleanup = () => document.removeEventListener('pointerdown', h);
    }, 0);
  }).catch(e => alert('Layout list failed: ' + e.message));
}
...
function widgetOptions(w){
  ...
  if (w.type==='bars' && !w.opts.hideScaleUI){
    const yGrid=el('input',{type:'number', value:w.opts.yGridLines||5, min:2, max:20, step:1, style:'width:60px'});
    yGrid.oninput=()=>{ w.opts.yGridLines=parseInt(yGrid.value)||5; };
...
function updateChartPopupValues(w, menu, buf, t0, xscale, plotL, ymin, ymax, yscale, cursorX){
  ...
  // TIME row (russ 8/26): Follow mode shows the chart time under the cursor,
  // Current mode shows the newest sample's chart time -- t_current already is
  // whichever sample the popup is reading. Rendered on the same clock as the
  // x-axis labels (rebased by _chartT0 when "reset clocks" is used).
  const tShow = t_current - (window._chartT0 || 0);
  tbody.append(el('tr',{}, [
    el('td',{style:'color:#7a8199'}, 'Time'),
    el('td',{}, tShow.toFixed(2) + ' s'),
    el('td',{className:'slope-cell', style: showSlope ? '' : 'display:none'}, '—')
  ]));
...
async function openConfigForm(providedConfig = null, banner = null){
  ...
  // ---- Remote access (server 2.13.24): FULL-ACCESS allowlist (russ 7/27:
  // "a list of permitted emails allowed full access, all others read only").
  // Row-list editor; the baked-in owner shows as a fixed row.
  cfg.controlUsers = (cfg.controlUsers || []).filter(Boolean);
  const ctlList = el('div', {});
  function renderCtlList() {
    ctlList.innerHTML = '';
    ctlList.append(el('div', {style: 'display:flex;align-items:center;gap:8px;margin:2px 0;color:var(--muted);font-size:12px'},
      [el('span', {style: 'width:280px'}, 'russ@keensystems.com'), el('span', {}, '(owner -- built in, always full access)')]));
    cfg.controlUsers.forEach((em, idx) => {
      const inp = el('input', {type: 'text', value: em, style: 'width:280px'});
      inp.oninput = () => { cfg.controlUsers[idx] = inp.value.trim().toLowerCase(); };
      const del = el('button', {className: 'btn danger', style: 'padding:2px 8px'}, 'X');
      del.onclick = () => { cfg.controlUsers.splice(idx, 1); renderCtlList(); };
      ctlList.append(el('div', {style: 'display:flex;align-items:center;gap:8px;margin:2px 0'}, [inp, del]));
    });
    const add = el('button', {className: 'btn', style: 'margin-top:4px'}, '+ Add email');
    add.onclick = () => { cfg.controlUsers.push(''); renderCtlList(); };
    ctlList.append(add);
  }
  renderCtlList();
  const restartBtn = el('button', {className: 'btn danger', style: 'margin-top:4px'}, 'Restart Server');
  restartBtn.onclick = () => {
    if (!confirm('Restart the control server?' + '\n\n' + 'Control stops for ~40 seconds, the rig lands in IDLE (RUN must be pressed again), and staged expression changes compile in. Browsers must reconnect.')) return;
    fetch('/api/server/restart', {method: 'POST'})
      .then(r => r.json())
      .then(d => alert(d.ok ? 'Restarting -- reconnect in ~40 s.' : 'Failed: ' + (d.error || '?')))
      .catch(() => alert('Restart issued (connection dropped as expected) -- reconnect in ~40 s.'));
  };
  const serverOps = fieldset('Server', el('div', {}, [
    el('p', {style: 'font-size:12px;color:var(--muted);margin-bottom:6px'},
      'Restarts the control service on the rig machine (applies staged expression/config changes). Not a Pi reboot.'),
    restartBtn
  ]));
  const remote = fieldset('Remote access -- full-access emails', el('div', {}, [
    el('p', {style: 'font-size:12px;color:var(--muted);margin-bottom:8px'},
      'Emails allowed FULL CONTROL when connecting remotely (Cloudflare login). Everyone else who logs in sees a view-only dashboard: watch everything, touch nothing (enforced by the server, not just hidden buttons). The local kiosk and LAN connections are never restricted. Tip: any browser can self-restrict by adding ?viewer=1 to the URL.'),
    ctlList
  ]));
  ...
  root.append(
    ...
    boards,logging,remote,serverOps,analogs,dig,aos,tcs,
...
async function openZeroAIDialog() {
  ...
  // Barometric helper (russ 7/28): local ambient arrives in INCHES OF MERCURY;
  // a non-zero entry fills Balance To with the psi equivalent (x 0.4911542 --
  // station pressure, NOT sea-level corrected). Opens at 0 every time.
  const inHgInput = el('input', {
    type: 'number',
    step: 'any',
    value: 0.0,
    style: 'width:100px;margin-left:8px'
  ...
  inHgInput.oninput = () => {
    const v = parseFloat(inHgInput.value);
    if (v) balanceInput.value = (v * 0.4911542).toFixed(4);
  };
  ...
  configRow.append(
    ...
    el('div', {style: 'flex:1;min-width:300px'}, [
      el('label', {}, ['Ambient (inHg): ', inHgInput]),
      el('div', {style: 'color:#7a7f8f;font-size:10px;margin-top:4px;margin-left:8px'}, 
        'Barometric inches of mercury → fills Balance To as psi (× 0.49115, station pressure)')
    ])
  );
