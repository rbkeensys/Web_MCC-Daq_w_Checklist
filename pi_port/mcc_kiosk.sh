...
exec nice -n 10 taskset -c 0-2 chromium --ozone-platform=wayland --password-store=basic --enable-wayland-ime   --wayland-text-input-version=3 --noerrdialogs --disable-session-crashed-bubble   --force-device-scale-factor=1.0 --disable-pinch --overscroll-history-navigation=0   --start-fullscreen --app=http://localhost:8000
