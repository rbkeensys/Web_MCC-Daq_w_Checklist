...
const ctrl = [
 ...
 ["condUseTotalizer","1","0/1","Distillate measurement: 1 = exact cumulative pulse totalizer (AI:CondTotal, count/K); 0 = integrate the windowed rate."],
...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      H3("Distillate measurement -- two methods (condUseTotalizer)"),
      P([new TextRun("yCondFlow is a "), code("counter_window_s"), new TextRun("-windowed RATE (CTR0 pulses, K=22000/L), recomputed ~once/sec and held between, sampled every tick -- so condEmptyDebounce must be >= ~2x that window. Two ways to measure the distillate:")]),
      EQ("condUseTotalizer = 1  (default):  condProduct = AI:CondTotal * condCalFactor * 1000"),
      EQ("    AI:CondTotal = cumulative pulse count / K (rollover-safe 32-bit, big-int sum) -> EXACT, no windowed-rate boundary error"),
      EQ("condUseTotalizer = 0:  condProduct += integral(yCondFlow) * condCalFactor   (windowed-rate integration, ~1 window error/batch)"),
      P("Both use the same CTR0 meter on the pump outlet; the totalizer is a second logical channel (counter_mode='total'). The pump batch cycle and the empty detection are identical either way -- the switch only changes the volume/rate source. condCalFactor (bench: known volume / metered pulses) corrects both."),
