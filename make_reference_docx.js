...
const startup = [
 ...
 ["EvapLowLevel (AI6) / evapLowMark","wet-dry / 750","0-5V / ml","LOW sight-glass sensor: heater-covered floor + wet-confirm to heat. WET above evapLowMark (sim); real: AI > 2.5V. Heater covered ~550ml (simHtrCoverML)."],
 ["EvapMidLevel (AI7) / evapMidMark","wet-dry / 2000","0-5V / ml","OPERATING-level sight-glass sensor (~3/4 up HX). The feed holds the level AT this mark via a slow wet/dry integral."],
 ["evapLevelSet / kFeedLevel / kFeedLevelI / levelDeadML","3025 / 0.015 / 0.0015 / 100","ml / (L/min/mL) / (L/min/s) / ml","Operating level = the 2900|3150 plateau edge = the measured 75%-full point (cal notes: 70%=2900, 80%=3150). Mid-edge INTEGRAL rides the edge; P outside the deadband recovers fill/upsets. Holds ~3019..3032 (band ~13mL) in sim."],
 ["Tank-gauge sender (AI6 EvapLevel)","cal table","V -> ml","33-240ohm arm-float sender + 550ohm (4x2.2k parallel) pullup to REGULATED 12V. AI6 slope -1.0 / offset 3.68 (KEEP: the cal is in these scaled volts; dry=0V, full ~3.0V, uh-oh-full 3600ml). Level-vs-ohms is strongly NONLINEAR (arm swing + tank geometry), so the decode is the MEASURED 14-plateau table (MVR Level Cal.csv) mapping scaled V -> plateau-center ml. Valid range -0.3..3.3V; outside (open wire = negative, short = 3.68) -> all switches DRY, no heat. Level switches derive from the decoded plateau: low 750 (587|850 edge), mid 3025 (2900|3150 edge = the measured 75%-FULL point = operating), high 3250 (trips on the 3.0V full plateau ~3325; spill into the demister only past ~3.8L). feedInvEst FUSED: dead-reckon + plateau-change midpoint snaps (teach the rate-bias) + span window (+/-senderSpanML 200, no teaching)."],
...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      BUL([b("PRIME: "), code("feedCmd = feedPrimeRpm"), new TextRun("; fill to the EvapLow mark (~0.75 L, heater covered) and advance to HEAT when "), code("feedInvEst >= feedPrimeML AND yEvapLow"), new TextRun(" -- the low sight-glass sensor CONFIRMS wet, so the heater never fires dry. If primed but EvapLow stays dry by "), code("evapLevelCheckML"), new TextRun(", Interlocks latches "), code("evapLevelErr"), new TextRun(" (feed-pump or sensor failure) -> trip + UI popup; clears on Stop.")]),
      BUL([b("HEAT: "), new TextRun("makeup heater on; the feed RAMPS the level from EvapLow up to the operating mark (EvapMid, ~2 L). Advance to RUN when "), code("yEvapTemp > evapTempSet - 3 AND yEvapMid"), new TextRun(" -- near boiling AND at the operating level (seed prodMeas = prodSetSafe so the blower doesn't spike).")]),
      BUL([b("RUN: "), new TextRun("closed-loop production; the feed HOLDS the level at EvapMid (see 2.2a).")]),

      H2("2.2a  Evaporator level control (FeedFollow)"),
      P([new TextRun("No continuous evaporator level sensor fits the design, so level is held with TWO wet/dry sight-glass sensors and the precisely-calibrated positive-displacement feed pump. Feed = measured-production feed-forward "), code("prodMeas x (1+blowdownFrac)"), new TextRun(" plus a SLOW integral on the EvapMid wet/dry state: wet -> trim the feed below boil-off (draw down), dry -> above (fill up). The level rides the mark with a slow toggle (~tens of minutes), and because the integral -- not the dead-reckon -- closes the loop, a mis-calibrated flow meter can NEVER run the level away (wet always pulls feed down). Far below the mark the feed fills at "), code("feedFillLpm"), new TextRun(". The dead-reckon "), code("feedInvEst"), new TextRun(" is clamped consistent with the sensor states each tick (for prime-verify + display), not used to hold level. Calibrate "), code("condCalFactor"), new TextRun(" once against the feed pump so production reads true.")]),
