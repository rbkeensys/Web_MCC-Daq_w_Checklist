...
const startup = [
 ...
 ["coldFeedTempC","95","degC","COLD-BOIL FILL LATCH (rig 7/25): below this steam temp there is no production for the FF to follow and no boil-off to relieve the level (the cold-crash fold even recenters feed to the learned base, cancelling downward level terms) -- so the feed goes bang-bang on the PHYSICAL sender: fill to the operating mark, then feed OFF, latched until the sender itself steps a tick back DOWN (real water consumed, not an estimate) or boiling begins. The cut bypasses the feed slew (immediate, like yEvapHigh). 0 disables."],
      ...
      ["feedHiEscS","30","s","RAW-HIGH ESCALATION (7/22): on the 2600 plateau the feed caps at 0.5xFF after the plateau persists feedHiOnS (10s, 7/28: an instant cut on boundary flicker injected the thermal disturbance behind the eT-excursion/makeup-fold/collapse cycle), 0.25xFF after feedHiEscS still high, OFF after 2x -- descent guaranteed regardless of FF bias (the confidence-softened 0.71xFF cap held 367rpm mean ON the plateau). Down-slew runs 4x (cuts are thermally free)."],
