...
const startup = [
 ...
 ["evapLevelSet / kFeedLevel / kFeedLevelI","2000 / 0.015 / 0.0015","ml / (L/min/mL) / (L/min/s)","Operating level = the EvapMid switch mark. The switch INTEGRAL holds the tight level; P acts only outside a +/-1-step (~200mL) deadband on the fused estimate (fast fill/upset recovery). Holds ~2.0L +/-21mL in sim."],
 ["Tank-gauge sender (senderVsup/Rser/RFull/REmpty/FullML/TravelMM/StepMM)","12 / 240 / 33 / 240 / 3000 / 317.5 / 21","V/ohm/ml/mm","12.5in US 33-240ohm sender reads the WHOLE HX (0-3L; LOW ohms = FULL, so voltage HIGH=empty ~6.0V, full ~1.45V). Decode inverts the divider to OHMS (linear in level; volts is not). 21mm steps = ~198mL quantization: too coarse to out-resolve the EvapMid switch at the setpoint, so it serves fill/monitoring/safety. feedInvEst is FUSED: dead-reckon + step-window clamps + step/switch edge snaps + a learned rate-bias (levelBiasLpm, self-calibrates the meter error against real level marks)."],
