...
const ctrl = [
 ["kpMakeup / kiMakeup","0.35 / 0.01","-","Makeup-heater PI gains. kp raised 0.05 -> 0.35 (rig 7/14): P was ~nothing so the integral did everything -- rail-to-rail duty, eT +/-2.5C. P leads, I trims."],
 ["kpSuper / kiSuper / kShFF","0.01 / 0.0005 / 0.03","- / - / duty/krpm","Superheat control = FLOW FEEDFORWARD + PI trim. The duty operating point scales with vapor mass flow (0.05 holds ~5C at 1500rpm, the same duty is nothing at 4000 -- the rig sagged into the wet-comp trip while the PI crawled), so kShFF*rpm/1000 carries it and the PI (retuned soft after the 0.05/0.01 gains bounced the element 173..209C) only trims; the PI error is CLAMPED to +/-10C (a cold pipe reads -74 -- not a deficit the cartridge can fix; unclamped it slammed duty to 1.0 and overshot the element to 269C) and the integral clamps +/-0.4 so it can cancel excess FF. The blower also actively RESCUES superheat: collapsed superheat at temp (vent closed) backs the rpm target off toward the run floor -- less mass flow = more superheat per watt."],
 ...
 ["condTimedEn / condTimedOnS / condTimedOffS","1 / 30 / 60","0-1 / s / s","SENSOR-BAD FALLBACK (optional): if the pump cycled condMeterFaultS with no real batch but the METER counted flow (>= 5ml), the CondLevel sensor is the liar (fog film) -> condSensErr popup + TIMED pumping for the rest of the run (ON condTimedOnS / OFF condTimedOffS). Timed cycles are CLEAN pump cycles, so batches + the batch rate come back despite the bad sensor. Meter counted nothing -> condMeterErr as before. Latched until Stop."],
 ...
 ["evapPressMin / blowerSeekRpmS / blowerSeekMaxRpm","13.0 / 15 / 2800","psia / rpm/s / rpm","Blower PRESSURE-SEEK (RUN, vent closed, BEFORE the first condensate batch): no measured production exists yet, so instead of freezing at the nominal 1500 the target CLIMBS at blowerSeekRpmS while the suction holds above evapPressMin+0.3 AND superheat is healthy (> minSuperheat+1) -- superheat collapse is the other 'outpacing the plant' signal -- CAPPED at blowerSeekMaxRpm (the rig boils hard enough that suction never sagged and the seek ran to the 4000 clamp). After batches, the production integral takes over (may exceed the cap) but never raises the target with suction at the floor; either mode backs off below the floor."],
