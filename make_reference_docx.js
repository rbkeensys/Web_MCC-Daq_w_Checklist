...
const startup = [
 ...
 ["evapLevelSet / kFeedLevel / kFeedLevelI / levelDeadML","1984 / 0.015 / 0.0015 / 50","ml / (L/min/mL) / (L/min/s) / ml","Operating level = the step-10 boundary (1984 = 10 x 198.4) -- the hold sits ON an observable step edge. Derived-mid-edge INTEGRAL + P outside the levelDeadML deadband on the fused estimate. Holds 1976..1992 (band ~16mL) in sim."],
 ["Tank-gauge sender (senderVsup/Rser/RFull/REmpty/FullML/TravelMM/StepMM)","12 / 550 / 33 / 240 / 3000 / 317.5 / 21","V/ohm/ml/mm","12.5in US 33-240ohm sender on AI6 (EvapLevel) is THE level sensing (sight-glass switches removed; AI7 spare). Pullup = 4 x 2.2k parallel (550) to regulated 12V: full ~0.68V, empty ~3.65V (<0.3V = unwired -> safe/dry). Decode inverts the divider to OHMS (linear in level; volts is not). ALL level switches (low 794 / mid 1984 / high 2380) derive from the quantized reading, marks ALIGNED to step boundaries. feedInvEst FUSED: dead-reckon + step-window clamps + edge snaps + learned rate-bias. Hold = mid-edge integral + P outside levelDeadML (50ml): ~2.0L +/-8mL in sim."],
