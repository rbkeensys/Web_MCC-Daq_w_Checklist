...
const ctrl = [
 ...
 ["evapPressMin / blowerSeekRpmS / blowerSeekMaxRpm","13.0 / 8 / 2800","psia / rpm/s / rpm","Blower PRESSURE-SEEK (RUN, vent closed, BEFORE the first condensate batch): no measured production exists yet, so instead of freezing at the nominal 1500 the target CLIMBS at blowerSeekRpmS while the suction holds above evapPressMin+0.3 AND superheat is healthy (> minSuperheat+1) -- superheat collapse is the other 'outpacing the plant' signal -- CAPPED at blowerSeekMaxRpm (the rig boils hard enough that suction never sagged and the seek ran to the 4000 clamp). After batches, the production integral takes over (may exceed the cap) but never raises the target with suction at the floor; either mode backs off below the floor."],
