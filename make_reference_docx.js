...
const ctrl = [
 ...
 ["prodFastWinS / prodFastAvgS","60 / 180","s / s","FAST production estimate (prodFast) for the FEED feed-forward: 60s totalizer windows into a 180s EMA (~2-3min response vs prodMeas ~10min). RUN only, NO proven-meter wait (a recompile resets that flag and left the fast FF blind at 0 while 0.6 L/min pumped out -- flow past the meter IS proof); cleared outside RUN. The BLOWER keeps the slow stable prodMeas -- two consumers, two time constants."],
