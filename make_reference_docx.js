...
const ctrl = [
 ...
 ["evapPressMin / blowerSeekRpmS","13.0 / 8","psia / rpm/s","Blower PRESSURE-SEEK (RUN, vent closed, BEFORE the first condensate batch): no measured production exists yet, so instead of freezing at the nominal 1500 the target CLIMBS at blowerSeekRpmS while the evap (suction) pressure holds above evapPressMin+0.3. Suction is the honest capability signal -- it only drops when the blower outpaces the boil. After batches, the production integral takes over but never raises the target with suction at the floor; either mode backs off below the floor."],
