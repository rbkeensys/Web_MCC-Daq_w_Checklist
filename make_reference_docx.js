...
const safety = [
 ["prodSetMax","0.5","L/min","productionSet rejected above this (or below 0); last valid kept (prodSetSafe). Raised from 0.3 after the rig measured 0.43 (the old clamp silently rejected productionSet=0.4)."],
 ...
 ["blowerMaxRpm","3500","rpm","Blower clamp (-> blowerRpmSafe). Lowered from 4000: in the stuck-hot deadlock the integral wound to 4000 and just churned the same steam hotter."],
 ["blowerRunMinRpm","1950","rpm","Production-control TURNDOWN FLOOR: once running, the blower target never drops below this -- below ~2000 there is no real vapor flow, superheat dies, and the run's thermal progress unwinds. If production exceeds productionSet at the floor, the feed simply matches it. (blowerMinRpm 800 remains the absolute floor used by gates elsewhere.)"],
 ["feedRpmMax","425","rpm","Feed-pump clamp (-> feedRpmSafe). Rig DOLILO 7/21: 300 -> 425 (375 also fine) -- at 0.4+ L/min production the feed must sustain ~400rpm just to match boil-off."],
 ...
 ["superHtrMax","275","C","Superheat-cartridge TC over-temp TRIP -- hard backstop above the 250C element regulation (the cartridge runs HOT by design in thin vapor). Latched."],
 ["superHtrRegC / kpShElem","250 / 0.05","C / duty/C","SH ELEMENT-TEMP REGULATION: duty capped so the cartridge holds ~250C until real vapor flow carries the heat away (poor coupling into thin vapor); then the superheat PI takes over. Raised 200 -> 250 per rig (200 couldn't keep up). Full duty 20C below target."],
 ["vaporOutSoft","115","C","Blower VAPOR-OUT FOLD-BACK: above this discharge temp all raises pause (resume below this-5) and the target backs off PROPORTIONALLY (0.3x just above, 3x at +30C -- the discharge climbs 20C in 30s, so a fixed rate sawtoothed and still let a 142C spike trip). The blower settles at the temperature-limited rpm."],
 ["prodStallLpm / prodStallS / ventPulseS","0.03 / 300 / 30","L/min / s / s","AIR-ACCUMULATION PURGE: RUN production stalled below prodStallLpm at temp for prodStallS -> the vent PULSES ventPulseS to dump the non-condensable blanket, repeating while stalled. Feed water carries dissolved air in continuously; air collects exactly on the condensing surface, and the steam side then runs 1-3 psi above Psat(T) -- the pressure-derived deltaT looks healthy while the true thermal dT is ~0 and nothing condenses (rig 7/15)."],
 ["ventReliefC","125","C","THERMAL-RELIEF vent: above this vapor-out the vent OPENS regardless of the ventDone air-purge latch -- dumps the superheated charge so fresh saturated vapor pulls through (superheated gas will not condense; the manual remedy that recovered the rig). Hysteresis: closes below this -10."],
 ...
 ["deltaTMin","2.5","C","MINIMUM COMPRESSION LIFT: deltaT (steamTsat - evapTsat) is the MVR's working head -- at ~1C the HX moves no heat, nothing condenses, and the sealed system just pressurizes. Below this (at temp, vent closed, HEAT or RUN) the blower target RAISES toward the seek cap; the superheat rescue only acts when lift is adequate."],
 ["feedStallRpm / feedStallMaxML","200 / 1500","rpm / ml","FEED-STALL guard: this much feed dead-reckoned NET of measured production at fill-rate commands without one UPWARD sender step -> the pump is not delivering (empty supply/tubing/stepper) or the sender is stuck -> latches evapLevelErr (trip + popup). Net basis (7/21): at high production the feed legitimately runs at fill-rate forever with a steady level -- feed matching production accumulates ~nothing; a dead feed line still counts at full rate."],
 ["tsatTrimC","2.0 (calibrate)","C","TSAT ALIGNMENT TRIM added to both computed Tsats: TCs + pressure transducers carry small offsets (rig 7/14 measured ~1.4; DOLILO 7/21 running value 2.0 adopted as default -- re-derive after the TC/PT zero cal). CALIBRATE at a vigorous VENTED boil: tsatTrimC = yEvapTemp - evapTsat. Cancels in deltaT; lowers superheatIn by the trim."],
 ["feedMinLpm","0.03","L/min","MINIMUM RUN FEED: the cold feed IS the subcooler's heat sink -- at feed 0 nothing condenses, production reads 0, the level never drops, and level control correctly asks for zero feed (a self-sustaining deadlock the level loop cannot see; rig 7/15). A trickle keeps the condensing path alive; the high-level cut overrides. The feedLevelTrim negative bound is -0.05 for the same reason (it only ever needs to shave feed-forward bias)."],
 ["feedSlewRpmS","15","rpm/s","AUTO-FEED SLEW LIMIT: production-measure jumps slammed the feed 0..450rpm, and every slam is a cold-water rush the makeup PI then chases (the 42s eT limit cycle). The level loop is slow anyway -- the command is rate-limited."],
 ["evapHighMark / evapHighWarnS","2750 / 3","ml / s","HIGH-LEVEL WARNING (2600|2900 plateau edge; E-TC DIN float ORed in): latched UI popup after 3s persistence, RUN-only and SWELL-QUALIFIED (7/22): the warn also requires recent-feed (feedRecentLpm 60s EMA) above feedWarnLpm (0.05) -- boil swell walks the reading over the edge with the feed idle (cold-start boil onset hit raw 3150 with feed OFF) and froth is not overfill. FeedFollow still cuts the feed INSTANTLY and unconditionally on yEvapHigh. Not during PURGE; not a hard trip."],
["evapHighClrS","60","s","Warning AUTO-CLEAR: the latched warn extinguishes after the level reads non-high continuously this long (it previously latched until stop and glowed all run after the startup expansion bob)."],
...
const ctrl = [
 ["kpMakeup / kiMakeup","0.5 / 0.03","-","Makeup-heater PI gains. 0.05 was integral-only (rail-to-rail duty, eT +/-2.5C); the user's 0.7/0.07 gave a fast 42s +/-1.2C limit cycle against the ~10s TC lag. 0.5/0.03 + the FEED SLEW LIMIT (the real disturbance was cold-water feed rushes) is the balance."],
 ["kpSuper / kiSuper / kShFF","0.2 / 0.002 / 0.03","- / - / duty/krpm","Superheat control = FLOW FEEDFORWARD + PI trim. The duty operating point scales with vapor mass flow (0.05 holds ~5C at 1500rpm, the same duty is nothing at 4000 -- the rig sagged into the wet-comp trip while the PI crawled), so kShFF*rpm/1000 carries it and the PI (retuned soft after the 0.05/0.01 gains bounced the element 173..209C) only trims; the PI error is CLAMPED to +/-10C (a cold pipe reads -74 -- not a deficit the cartridge can fix; unclamped it slammed duty to 1.0 and overshot the element to 269C) and the integral clamps +/-0.4 so it can cancel excess FF. The blower also actively RESCUES superheat: collapsed superheat at temp (vent closed) backs the rpm target off toward the run floor -- less mass flow = more superheat per watt."],
 ["kProd","20","rpm/(L/min.s)","Blower production-control gain. Anti-windup: the target is not raised while the soft-start/ramped command still lags it. Steers on the FAST estimate (prodFast) when available -- the double-smoothed prodMeas kept it climbing on stale data (rig 7/21: 1950 -> 2860 with production already over set). Up-rate capped at err 0.15 (3rpm/s)."],
 ["kProdDown / kProdBand","4 / 0.03","x / L/min","OVERSHOOT UNWIND + RAISE DEADBAND (rig 7/21): production above set falls at kProd*kProdDown (2860 -> 2100 in ~90s instead of ~6min of overselling at +30%); raising stops within kProdBand of set so momentum + measurement lag close the gap instead of overshooting it."],
 ["kFeedLevel / kFeedLevelI","0.015 / 0.0015","(L/min)/ml, (L/min)/s","Feed level P gain (active OUTSIDE the deadband: fill + upset recovery) / slow edge-ride integral (INSIDE the deadband; the tight hold). ANTI-WINDUP (7/21): the trim also unwinds at normal rate while the level is ABOVE the deadband and bleeds at 4x during a high-level cut -- frozen trim wound up in a trough used to power the overfill that followed."],
 ["feedTrimMax","0.12","L/min","Edge-integral trim UPPER bound. Was the full feed range (0.46!) -- the trim wound to +0.20 covering the 7/21 prodMeas lag and then overfed the level to 2900 repeatedly. The trim shaves feed-forward bias; production belongs to the FF. (No-sender fallback keeps the wide bound: there the trim IS the feed.)"],
 ["prodFastWinS / prodFastAvgS","300 cap / 180","s / s","FAST production estimate (prodFast), CYCLE-SYNCED (7/22): the window closes at each PUMP-OFF edge -- the reservoir is in the same just-emptied state cycle to cycle, so volume/period is EXACT. Fixed 60s windows ALIASED against the 400-750s flow-follow pump cycles (prodFast swung 0.19-0.47 while true per-cycle production sat flat at 0.46; the blower hunted +/-900rpm, ~10min period, chasing it). prodFastWinS is only the CAP (continuous pumping/stall); prodFastMinS (45s) ignores micro-cycles. Feeds the feed FF AND the blower loop (pmG). RUN only, NO proven-meter wait (a recompile resets that flag and left the fast FF blind at 0 while 0.6 L/min pumped out -- flow past the meter IS proof); cleared outside RUN. The BLOWER keeps the slow stable prodMeas -- two consumers, two time constants."],
 ...
 ["prodUseInput / prodInWinS","0 / 240","0-1 / s","INPUT-SIDE production measurement OPTION: prod = calibrated feed IN minus the sender-anchored level CHANGE over the window -- only the positive-displacement pump + fused level, nothing from the condensate meter (for when the output measurement can't be trusted; in sim the input estimate beat a 12%-miscalibrated meter). prodUseInput=1 selects it for the blower production control; 0 = output measurement (default). All downstream logic (integral, floor, seek, caps) shared."],
 ["prodWinS / prodFromTotal","180 / 1","s / 0-1","TOTALIZER-WINDOW production rate: with prodFromTotal=1 (default) condRateLpm comes SOLELY from the cumulative meter delta over this rolling window -- needs NO pump cycles (a stuck-wet CondLevel micro-cycles the pump so no batch ever reaches condRateMinML; the rig sat blind at the seek cap with a WORKING meter). Batches still refresh the same rate more precisely when they land; >= 5ml/window required, plausibility-capped."],
 ["prodAvgS","120","s","Production BURST-AVERAGING horizon: every totalizer window feeds a time-weighted EMA over this, so pump-out bursts and quiet windows average to the TRUE rate. Rig DOLILO 7/21: 600 -> 120 (blower tracks prodSet 0.35 at avg ~0.36); ~60 is plausible since the pump cycles 2x/min in production."],
 ["condTimedEn / condTimedOnS / condTimedOffS / condTimedMinLpm / condTimedAlways","1 / 5 / 30 / 0.05 / 1","-","TIMED FLOW-FOLLOW pumping (rig-proven): probe every condTimedOffS (30s), run at least condTimedOnS (5s), then KEEP pumping while the 5s-avg meter flow holds above condTimedMinLpm -- the METER decides when the reservoir is empty, not the fogged CondLevel sensor. condTimedAlways=1 (DEFAULT) = primary strategy from RUN start; 0 = sensor-driven with this as the automatic sensor-bad fallback (condSensErr popup: pump cycled condMeterFaultS with no real batch but the meter counted flow). Timed cycles are CLEAN pump cycles, so batches + the batch rate still work."],
 ...
 ["ventFitted","0","0/1","Powered vent valve PLUMBED? 0 (rig reality 7/22): only the always-open needle bleed at the HX-output bottom -- the vent state machine is bypassed (closed+done), because a phantom venting state boosts the blower, freezes production control, and disarms the superheat rescue (the 7/22 wet-comp trip). Sim always models the vent. Set 1 when the powered vent lands at the HX bottom."],
 ["ventOpenTemp / ventCloseTemp","97 / 100","C","Startup air-purge vent: OPEN below / CLOSE above this blower-out vapor temp (yVaporOut). Lowered from 98/101 -- the rig never reached 101 at the vent TC, so it sat open bleeding steam. Once closed it LATCHES closed for the rest of the run (ventDone): the air purge is once-per-run; a mid-run upset cooling yVaporOut must not re-open it. ProductionControl does not integrate while the vent is open (production is being vented -- the meter can't see it)."],
 ["ventBoostRpm","2500","rpm","VENT-PURGE BOOST: blower target while the vent is open with the evaporator at temp (>= set-3). At the nominal 1500 the vapor mass flow is too thin to heat the discharge path -- VaporOut crawled 65->84C in 14min on the rig and the vent never closed. Boost shoves mass through; when the vent closes the target is SEEDED at the boost rpm (hand-off, no step down -- dropping 2500->1500 crashed the progress on the rig) and glides to the natural operating point under production/pressure control."],
 ["evapPressMin / blowerSeekRpmS / blowerSeekMaxRpm","13.0 / 25 / 2800","psia / rpm/s / rpm","Blower PRESSURE-SEEK (RUN, vent closed, BEFORE the first condensate batch): no measured production exists yet, so instead of freezing at the nominal 1500 the target CLIMBS at blowerSeekRpmS while the suction holds above evapPressMin+0.3 AND superheat is healthy (> minSuperheat+1) -- superheat collapse is the other 'outpacing the plant' signal -- CAPPED at blowerSeekMaxRpm (the rig boils hard enough that suction never sagged and the seek ran to the 4000 clamp). After batches, the production integral takes over (may exceed the cap) but never raises the target with suction at the floor; either mode backs off below the floor."],
...
const startup = [
 ...
 ["feedPrimeRpm","400","rpm","Feed rpm during the PRIME fill. Rig DOLILO 7/21: 250 -> 400."],
 ...
 ["feedBaseTau / feedBaseRampS","240 / 600","s / s","ADAPTIVE FEED BASE (rig 7/21): feedBaseLpm = slow EMA of the COMMANDED feed -- the level is held in a band, so the long-run average feed IS the balance rate (calibrated pump side: immune to meter cal error and prodFast burst aliasing). Confidence w ramps 0 -> 0.7 over feedBaseRampS of RUN time: FF becomes blend(prodFast, base) and the raw-edge override amplitudes soften by 1-0.6w (low fill +0.20 -> +0.12; high cut 0.5xFF -> 0.71xFF). Cure for the 425<->190rpm bang-bang whose 425 slams were cold slugs the makeup fought at full duty. Base retained across stop/start; each run re-proves the ramp."],
 ["feedHiEscS","30","s","RAW-HIGH ESCALATION (7/22): on the 2600 plateau the feed caps at 0.5xFF immediately, 0.25xFF after this long still high, OFF after 2x -- descent guaranteed regardless of FF bias (the confidence-softened 0.71xFF cap held 367rpm mean ON the plateau). Down-slew runs 4x (cuts are thermally free)."],
 ["feedHighRawML","100","ml","RAW-HIGH THROTTLE (rig 7/21): raw sender ABOVE evapLevelSet + this (= stepped onto the 2600 plateau) caps the feed at HALF production-match and bleeds the trim at 2x -- the level drifts back down NOW instead of coasting into the 2750 warn (which remains the full-cut backstop). Sim steady band 100-200ml -> 15ml: the plateau edges, not the estimates, bound the level on both sides."],
 ["feedLowRawML","300","ml","RAW-LOW OVERRIDE (rig 7/21 restart): in RUN, the raw sender below evapLevelSet - this (>= one plateau genuinely low) forces fill-rate feed (feedFF + feedFillLpm) over every cap and fold. The physical sensor outranks all estimates: low is LOW. Sits after the cold-crash fold -- an emptying evaporator (dry heater) outranks a temperature sag."],
 ["fillStopML","200","ml","HEAT fills COLD water to evapMidMark - this, and (7/22) a COLD-FILL CAP holds it there: level-P + trim used to walk the cold charge up to the mark anyway, and boil-onset swell (+500ml froth on this rig) launched from 2500 to raw 3150. Above the reduced target in HEAT the feed caps at FF (~0) and trim wind-up is frozen; expansion + swell carry the level the rest, RUN tops off under the sensors. HEAT->RUN advances on temp + raw >= mark-fillStopML."],
...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      EQ("pm  = prodFast (fast, ~60-90s) when available, else prodMeas"),
      EQ("err = prodSetSafe - pm    ; err in (0, kProdBand) -> 0 ; err > 0.15 -> 0.15 ; err < 0 -> err * kProdDown"),
      EQ("blowerRpmSet = clamp( blowerRpmSet + kProd*err*cdt , min , max )"),
      ...
      EQ("feedFF  = (prodFast > 0 ? prodFast : prodMeas) * (1+blowdownFrac)"),
      EQ("feedLpm = feedFF + P(fused est, outside deadband) + feedLevelTrim"),
      ...
      P([b("ADAPTIVE BASE (rig 7/21 evening): "), new TextRun("the commanded-feed EMA "), code("feedBaseLpm"), new TextRun(" learns the true balance rate (average feed = boil-off while the level is band-held); as RUN-time confidence grows the FF blends toward it (meter noise/cal error attenuated up to 70%) and the raw-edge overrides soften from bang-bang (425<->190rpm, cold-slug slams) to ripples around the base.")]),
      P([b("FAST FEED-FORWARD (rig 7/21): "), new TextRun("the feed follows "), code("prodFast"), new TextRun(" (60s meter windows, 180s EMA) instead of the ~10min-lagging prodMeas: the slow FF starved a production surge (level crash 2600->1500), wound the trim up covering it, then FF-catch-up + fat trim overfed to 2900 in a sawtooth. The fast estimate keeps the FF honest within ~2-3min in both directions; the blower keeps the slow prodMeas for stability. FeedFollow is also compiled as FLAT GUARDED SECTIONS after a transpiler hoisting miscompile put its tail outside the autoFeed guard.")]),
      ...
      EQ("energyProdL += yCondFlow * condCalFactor * dt/60   ; specKWhL = energyKWh / energyProdL"),
      P([b("Specific energy (kWh/L): "), new TextRun("a dashboard Button widget (VARIABLE mode) named "), code("resetEnergyCalcs"), new TextRun(" zeroes "), code("energyKWh"), new TextRun(" and the window product totalizer "), code("energyProdL"), new TextRun(" TOGETHER, so both cover the same window -- press+release at RUN entry and "), code("specKWhL"), new TextRun(" reads live kWh-per-L for the run (guarded until ~50ml lands). The MVR figure of merit: raw resistive distillation ~0.63 kWh/L; this machine targets ~0.1-0.15. Power displays as powerW (watts); energy totals in kWh.")]),
