...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      P([new TextRun("yCondFlow is a "), code("counter_window_s"), new TextRun("-windowed rate (CTR0 pulses, K=22000/L), refreshed ~once/sec and held between -- so condEmptyDebounce must be >= ~2x that window. For an exact lifetime total, read the cumulative pulse count directly (meter on the pump outlet + stops on air => count/K = distillate).")]),
