...
class AppConfig(BaseModel):
    ...
    # ---- Logging mode (2.5.0): 'full' = every tick, one session.csv (dev);
    # 'eco' = decimated to logHz with daily-rotated files + retention --
    # for self-hosted rigs (Raspberry Pi on SD/limited storage).
    logMode: Optional[str] = "full"
    logHz: Optional[float] = 1.0
    logRetainDays: Optional[int] = 30
    viewOnlyUsers: list = []   # Cloudflare Access emails that may WATCH but not control (2.13.23)
    controlUsers: list = []    # CONTROL ALLOWLIST (2.13.24): when non-empty, ONLY these Access emails may control; all other authenticated visitors are view-only
