                        ...
                        if UlDigitalDir is not None:
                            for _b in range(8):
                                try:
                                    ...
                                    # NEVER silent (7/27: bits 2-7 stayed INPUTS after a
                                    # power-cycle + stale-session recovery -- floating pins
                                    # read LOW and the active-low relay board ran the pumps
                                    # with software commanding OFF the whole time)
                                    print(f"[MCCBridge] DO bit {_b} OUTPUT config FAILED: {_e}")
                    self._ul_1608[bn] = h
                    got = True
    ...
    def set_do(self, index: int, state: bool, active_high=None):
        ...
        if board_num in self._ul_1608:
            ...
            try:
                ...
                # WATCHDOG FOOD: a RUN of consecutive failures on an OPEN handle is
                # the wedged-handle signature (7/24 400C incident) -- the server's
                # health monitor stops petting systemd past _do_fail_streak ~500 and
                # the restart rebuilds the handles (open() is idempotent since 2.6.3).
                self._do_fail_streak = getattr(self, "_do_fail_streak", 0) + 1
