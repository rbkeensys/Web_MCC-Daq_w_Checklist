...
class CPPCodeGenerator:
    ...
    def generate_node(self, node) -> str:
        ...
        # Assignments
            ...
            # NEGATE emits "(-1.0)" -- accept the parenthesized negative literal too,
            # else "static.x = -1" at top level compiles as a RUNTIME assignment that
            # re-runs EVERY tick and silently wipes the var (bit purgeZeroML + levelStepPrev).
            import re
            _s = expr.strip()
            is_constant = bool(re.match(r'^-?\d+\.?\d*$', _s)) or bool(re.match(r'^\(-\d+\.?\d*\)$', _s))
            ...
            expr = self.generate_node(node.children[0]) if node.children else "0.0"
            # Emit the RAW value; the server thresholds (>=1.0) for normal DOs and
            # uses it as the 0..1 duty for PWM-mode DOs.
            return f"result = do_out[{idx}] = ({expr});"
        ...
        # Function calls
            ...
            # Map metalanguage names that aren't <cmath> to our runtime helpers
            # (rand/randn -> normalized [0,1)/normal, sign). floor/ceil/round are
            # in <cmath> and pass through.
            _FN_MAP = {'rand': 'expr_rand', 'randn': 'expr_randn', 'sign': 'expr_sign'}
            mapped = _FN_MAP.get(func_name, func_name)
            # Direct <cmath>/helper functions pass through unchanged.
...
// rand()/randn()/sign() for expressions. xorshift32 PRNG (deterministic per
// process load, good sequence within a run -- reproducible, no time dependency).
inline unsigned int& _expr_rng() { static unsigned int s = 2463534242u; return s; }
inline double expr_rand() {                      // uniform [0,1)
    unsigned int& s = _expr_rng();
    s ^= s << 13; s ^= s >> 17; s ^= s << 5;
    return (double)s / 4294967296.0;
}
inline double expr_randn() {                     // standard normal (Box-Muller)
    double u1 = expr_rand(); if (u1 < 1e-12) u1 = 1e-12;
    double u2 = expr_rand();
