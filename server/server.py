...
def put_expressions(body: dict):
    ...
    try:
        ...
        if USE_CPP_EXPRESSIONS or cpp_backend is not None:
            log.info("[CPP-EXPR] Expressions saved, recompiling (old DLL keeps control)...")
            old_backend = cpp_backend

            if not compile_cpp_expressions():
                log.error("[CPP-EXPR] Compile FAILED -- previous expressions remain in control")
                return {"ok": False,
                        "error": "compile failed; the PREVIOUS expressions are still running"}

            new_backend = load_cpp_backend(dll_path=CURRENT_DLL_PATH)
            if not new_backend:
                log.error("[CPP-EXPR] Reload FAILED -- previous expressions remain in control")
                return {"ok": False,
                        "error": "compiled but reload failed; the PREVIOUS expressions are still running"}
