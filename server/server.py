...
def _install_console_fd_capture():
    ...
    # FROZEN-APP FALLBACK (2.13.29): a PyInstaller exe launched without a
    # usable console (spawned by the windowed installer/launcher with
    # CREATE_NO_WINDOW, some shortcut configurations, service contexts) has
    # INVALID std handles -- os.dup(1) raises, the whole install aborted,
    # and the web console widget silently got nothing (the compiled-vs-
    # PyCharm "prints don't show" mystery). There is no terminal to echo to
    # in that case, so echo to devnull and carry on: the PIPES are what
    # feed the browser console, and dup2 CREATES fd 1/2 if they don't exist.
    try:
        saved_out = os.dup(1)
    except OSError:
        saved_out = os.open(os.devnull, os.O_WRONLY)
    try:
        saved_err = os.dup(2)
    except OSError:
        try:
            saved_err = os.open(os.devnull, os.O_WRONLY)
        except OSError:
            os.close(saved_out)
            raise
...
try:
    ...
    _CONSOLE_CAPTURE_ERR = repr(_cap_err)
...
# OWNER identities (russ 7/27): ALWAYS full control, baked into code -- no
# config edit, emptied list, or UI accident can ever lock the owner out.
OWNER_EMAILS = {"russ@keensystems.com"}

def _is_view_only(headers) -> bool:
    try:
        email = (headers.get("cf-access-authenticated-user-email") or "").strip().lower()
        if not email:
            return False            # local/LAN (no tunnel identity): never restricted
        if email in OWNER_EMAILS:
            return False            # owner: unconditional control
        view = [str(x).strip().lower() for x in (getattr(app_cfg, "viewOnlyUsers", None) or [])]
        if email in view:
            return True
        ctl = [str(x).strip().lower() for x in (getattr(app_cfg, "controlUsers", None) or [])]
        if ctl:
            # CONTROL ALLOWLIST mode (2.13.24, russ): with any entries present,
            # ONLY those identities may control -- every other authenticated
            # visitor (Access policy set to Everyone + one-time PIN) is
            # automatically view-only. Public dashboard, private controls.
            return email not in ctl
        return False
    except Exception:
        return False
...
def api_diag():
    ...
    return {
        ...
        "console_capture": bool(_CONSOLE_CAPTURE_OK),       # 2.13.29: False = expression prints can't reach the web console; see console_capture_err
        "console_capture_err": _CONSOLE_CAPTURE_ERR,
        "board1608": b1608,
        "boardetc": betc,
    }
...
# ---- MACHINE LIFETIME TOTALS (russ 7/28): energyKWh + energyProdL persist in
# machineTotals.json -- loaded into the statics at boot, saved once a minute
# (atomic tmp+rename; trivial SD wear). These are MONOTONIC odometers: only
# EDITING THE FILE (server stopped) legitimately resets them, so any observed
# drop (statics reset from an expression save, button, etc.) is treated as
# illegitimate and the saved values are RESTORED instead of overwritten.
MACHINE_TOTALS_KEYS = ("energyKWh", "energyProdL")

def _totals_backend_get():
    try:
        if cpp_backend and hasattr(cpp_backend, "staticvar_map"):
            out = {}
            for k in MACHINE_TOTALS_KEYS:
                i = cpp_backend.staticvar_map.get(k)
                if i is None:
                    return None
                v = float(cpp_backend.static_vars[i])
                out[k] = v if v == v else 0.0   # NaN guard
            return out
    except Exception:
        pass
    return None

def _totals_backend_set(vals):
    try:
        if cpp_backend and hasattr(cpp_backend, "staticvar_map"):
            for k in MACHINE_TOTALS_KEYS:
                i = cpp_backend.staticvar_map.get(k)
                if i is not None:
                    cpp_backend.static_vars[i] = float(vals.get(k, 0.0))
            return True
    except Exception as e:
        print(f"[TOTALS] backend set failed: {e}")
    return False

_machine_totals = {k: 0.0 for k in MACHINE_TOTALS_KEYS}

def _load_machine_totals():
    global _machine_totals
    p = CFG_DIR / "machineTotals.json"
    try:
        if p.exists():
            d = json.loads(p.read_text(encoding="utf-8"))
            _machine_totals = {k: float(d.get(k, 0.0)) for k in MACHINE_TOTALS_KEYS}
    except Exception as e:
        print(f"[TOTALS] load failed (starting from zero): {e}")
    if _totals_backend_set(_machine_totals):
        print("[TOTALS] lifetime totals loaded: " +
              ", ".join(f"{k}={_machine_totals[k]:.4f}" for k in MACHINE_TOTALS_KEYS))

def _save_machine_totals():
    p = CFG_DIR / "machineTotals.json"
    tmp = CFG_DIR / "machineTotals.json.tmp"
    body = dict(_machine_totals)
    body["note"] = "MACHINE LIFETIME totals -- edit ONLY with the server stopped to reset"
    tmp.write_text(json.dumps(body, indent=2))
    os.replace(str(tmp), str(p))

async def _machine_totals_task():
    global _machine_totals
    while True:
        await asyncio.sleep(60)
        cur = _totals_backend_get()
        if cur is None:
            continue
        dropped = any(cur[k] < _machine_totals[k] - 0.0005 for k in MACHINE_TOTALS_KEYS)
        if dropped:
            # statics were reset out from under us -- restore, never persist a wipe
            _totals_backend_set(_machine_totals)
            print("[TOTALS] statics reset detected -- lifetime totals RESTORED")
            continue
        changed = any(abs(cur[k] - _machine_totals[k]) > 0.0005 for k in MACHINE_TOTALS_KEYS)
        if changed:
            _machine_totals = cur
            try:
                _save_machine_totals()
            except Exception as e:
                print(f"[TOTALS] save failed: {e}")
...
async def _boot_safety():
    ...
    _load_machine_totals()
    asyncio.create_task(_machine_totals_task())
    asyncio.create_task(_disk_guard_task())

    # ACQ AT BOOT (2.13.35): acquisition used to start on the FIRST browser
    # connection -- headless restarts (9/4, 9/11 x2) sat with hardware open
    # but no ticks, no control evaluation, and a 0-byte log until someone
    # happened to open the UI. A rig server should acquire from boot; it is
    # safe because restarts land IDLE (runSystem=0, DO safe-init) so nothing
    # actuates until commanded. The WS-connect start remains as a fallback.
    async def _acq_at_boot():
        global run_task
        await asyncio.sleep(3)   # let hw safe-init settle
        if run_task is None or run_task.done():
            print("[MCC-Hub] starting acquisition at boot (no client needed)")
            run_task = asyncio.create_task(acq_loop())
    asyncio.create_task(_acq_at_boot())
    def _hw_init():
        try:
            mcc.open(app_cfg)
...
def restart_server():
    """UI-triggered service restart (config page, 2.13.27). Detached so the
    HTTP response gets out before systemd recycles us. Linux/systemd only.
    View-only accounts are blocked by the mutating-API middleware."""
    if os.name == "nt":
        return {"ok": False, "error": "service restart is a Pi/systemd feature"}
    try:
        import subprocess
        subprocess.Popen(["sudo", "-n", "bash", "-c", "sleep 1; systemctl restart mcc-server"],
                         start_new_session=True)
        print("[SERVER] UI-requested service restart in 1s")
        return {"ok": True}
    except Exception as e:
        return {"ok": False, "error": str(e)}
...
# ---- SHARED LAYOUT LIBRARY (2.13.28, russ 7/29): named layouts stored on the
# server so ANY visitor (full control or view-only) can browse + load them --
# no file shipping. Loading is a GET (open to viewers; applying a layout is a
# purely local act); publish/delete are mutating verbs, so the view-only
# middleware already 403s them for non-control identities. The last-used
# (localStorage) and first-time default (layout_default.json) behaviors are
# untouched -- the library is opt-in via the Layouts menu.
LAYOUTS_DIR = CFG_DIR / "layouts"

def _layout_slug(name: str) -> str:
    import re as _re
    s = _re.sub(r"[^A-Za-z0-9 _\-]", "", str(name)).strip()[:48]
    return s.replace(" ", "_")

@app.get("/api/layouts")
def list_shared_layouts():
    try:
        if not LAYOUTS_DIR.exists():
            return {"ok": True, "names": []}
        names = sorted(p.stem.replace("_", " ") for p in LAYOUTS_DIR.glob("*.json"))
        return {"ok": True, "names": names}
    except Exception as e:
        return {"ok": False, "error": str(e)}

@app.get("/api/layouts/{name}")
def get_shared_layout(name: str):
    slug = _layout_slug(name)
    p = LAYOUTS_DIR / (slug + ".json")
    if not slug or not p.exists():
        return {"ok": False, "error": "no such layout"}
    try:
        return json.loads(p.read_text(encoding="utf-8"))
    except Exception as e:
        return {"ok": False, "error": str(e)}
...
def put_shared_layout(name: str, body: dict):
    slug = _layout_slug(name)
    if not slug:
    ...
    try:
        ...
        import os as _os
        _os.replace(tmp, p)
        print(f"[Layout] shared layout '{slug}' saved")
        return {"ok": True, "name": slug.replace("_", " ")}
    except Exception as e:
        return {"ok": False, "error": str(e)}

@app.delete("/api/layouts/{name}")
def delete_shared_layout(name: str):
    slug = _layout_slug(name)
    p = LAYOUTS_DIR / (slug + ".json")
    if not slug or not p.exists():
        return {"ok": False, "error": "no such layout"}
    try:
        p.unlink()
        print(f"[Layout] shared layout '{slug}' deleted")
        return {"ok": True}
    except Exception as e:
        return {"ok": False, "error": str(e)}

# ---- SHARED CHECKLIST LIBRARY (2.13.30) --------------------------------
# The checklist FILE no longer has to be hand-copied between computers:
# whenever any browser loads a checklist .txt locally, the widget publishes
# it here (PUT), and every other computer can pull it with one click
# (Shared button -> GET list -> GET text). Mirrors the shared-layout
# library: GETs are open (loading is a local act), PUT/DELETE are mutating
# verbs so the view-only middleware already 403s non-control identities.
CHECKLISTS_DIR = CFG_DIR / "checklists"

@app.get("/api/checklists")
def list_shared_checklists():
    try:
        if not CHECKLISTS_DIR.exists():
            return {"ok": True, "names": []}
        names = sorted(p.stem.replace("_", " ") for p in CHECKLISTS_DIR.glob("*.txt"))
        return {"ok": True, "names": names}
    except Exception as e:
        return {"ok": False, "error": str(e)}

@app.get("/api/checklists/{name}")
def get_shared_checklist(name: str):
    slug = _layout_slug(name)
    p = CHECKLISTS_DIR / (slug + ".txt")
    if not slug or not p.exists():
        return {"ok": False, "error": "no such checklist"}
    try:
        return {"ok": True, "name": slug.replace("_", " "),
...
def put_shared_checklist(name: str, body: dict):
    slug = _layout_slug(name)
    if not slug:
        return {"ok": False, "error": "bad name"}
    text = body.get("text")
    if not isinstance(text, str) or not text.strip():
        return {"ok": False, "error": "empty checklist"}
    try:
        ...
        import os as _os
        _os.replace(tmp, p)
        print(f"[Checklist] shared checklist '{slug}' saved ({len(text)} chars)")
        return {"ok": True, "name": slug.replace("_", " ")}
    except Exception as e:
        return {"ok": False, "error": str(e)}

@app.delete("/api/checklists/{name}")
def delete_shared_checklist(name: str):
    slug = _layout_slug(name)
    p = CHECKLISTS_DIR / (slug + ".txt")
    if not slug or not p.exists():
        return {"ok": False, "error": "no such checklist"}
    try:
        p.unlink()
        print(f"[Checklist] shared checklist '{slug}' deleted")
        return {"ok": True}
    except Exception as e:
        return {"ok": False, "error": str(e)}


# ---- SD/DISK SPACE GUARD (2.13.31) -------------------------------------
# The 9/7 incident: a 14GB full-rate log silently filled the SD card -- the
# logger died mid-run and /api/logs/close 500'd (ENOSPC). Now the server
# checks the logs volume every minute; at >=90%% it broadcasts a disk_alert
# to EVERY connected browser (and carries it in the WS hello for late
# joiners). The modal on any computer can (a) download the closed sessions
# then delete them, or (b) delete oldest until below target. Space recovery
# clears the alert on ALL watchers via the next broadcast.
_disk_alert = {"active": False, "pct": 0.0, "free_gb": 0.0}
_DISK_ALERT_PCT = 90.0
_DISK_CLEAR_PCT = 88.0

def _disk_usage_pct():
    import shutil as _sh
    u = _sh.disk_usage(str(LOGS_DIR))
    return (u.used / u.total * 100.0, u.free / 1e9)

def _closed_log_sessions():
    """Log session dirs EXCLUDING the active one, oldest first."""
    import time as _t
    active = None
    try:
        if session_logger is not None:
            active = os.path.basename(str(session_logger.folder))
    except Exception:
        pass
    out = []
    try:
        for d in sorted((x for x in LOGS_DIR.iterdir() if x.is_dir()),
                        key=lambda x: x.stat().st_mtime):
            if d.name == active:
                continue
            if d.stat().st_mtime > _t.time() - 180:
                continue  # belt-and-suspenders: never touch a dir being written
            files = [f for f in sorted(d.iterdir()) if f.is_file()]
            out.append({"name": d.name,
                        "bytes": sum(f.stat().st_size for f in files),
                        "files": [f.name for f in files]})
    except Exception as e:
        print(f"[DiskGuard] session list failed: {e}")
    return out

async def _disk_guard_task():
    global _disk_alert
    while True:
        try:
            pct, free_gb = _disk_usage_pct()
            was = _disk_alert["active"]
            if pct >= _DISK_ALERT_PCT:
                _disk_alert = {"active": True, "pct": round(pct, 1), "free_gb": round(free_gb, 2)}
                await broadcast({"type": "disk_alert", **_disk_alert})
                if not was:
                    print(f"[DiskGuard] ALERT: logs volume {pct:.1f}% full ({free_gb:.2f} GB free)")
            elif was and pct < _DISK_CLEAR_PCT:
                _disk_alert = {"active": False, "pct": round(pct, 1), "free_gb": round(free_gb, 2)}
                await broadcast({"type": "disk_alert", **_disk_alert})
                print(f"[DiskGuard] cleared: logs volume {pct:.1f}% full")
        except Exception as e:
            print(f"[DiskGuard] check failed: {e}")
        await asyncio.sleep(60)
...
def logs_sessions():
    ...
    act = None
    try:
        # ACTIVE session too (2.13.34) -- the local-viewer flow wants the log
        # of the run being watched; a mid-write CSV snapshot reads fine.
        if session_logger is not None:
            d = LOGS_DIR / os.path.basename(str(session_logger.folder))
            if d.is_dir():
                files = [f for f in sorted(d.iterdir()) if f.is_file()]
                act = {"name": d.name,
                       "bytes": sum(f.stat().st_size for f in files),
        ...
        pass
    return {"ok": True, "pct": round(pct, 1), "free_gb": round(free_gb, 2),
            "alert": _disk_alert["active"], "active": act,
            "sessions": _closed_log_sessions()}

@app.get("/api/logs/viewer_scales")
def logs_viewer_scales():
    p = LOGS_DIR / "_viewer_scales.json"
    if not p.is_file():
        return {"ok": False, "error": "no viewer bundle yet"}
    return FileResponse(str(p), media_type="application/json")

@app.get("/api/logs/download/{session}/{fname}")
def logs_download(session: str, fname: str):
    if any(c in session + fname for c in ('/', '\\', '..')):
        return {"ok": False, "error": "bad path"}
    tgt = LOGS_DIR / session / fname
    if not tgt.is_file():
        return {"ok": False, "error": "no such file"}
    return FileResponse(str(tgt), filename=f"{session}_{fname}",
                        media_type="application/octet-stream")

@app.post("/api/logs/prune")
async def logs_prune(body: dict = None):
    """Delete oldest CLOSED sessions until the volume is below target_pct
    (default 85). body {"all": true} deletes every closed session (the
    download-then-clear path). The active session is never touched."""
    global _disk_alert
    import shutil as _sh
    body = body or {}
    target = -1.0 if body.get("all") else float(body.get("target_pct", 85))
    deleted = []
    for sess in _closed_log_sessions():
        pct, _free = _disk_usage_pct()
        if target >= 0 and pct <= target:
            break
        _sh.rmtree(LOGS_DIR / sess["name"], ignore_errors=True)
        deleted.append(sess["name"])
        print(f"[DiskGuard] pruned {sess['name']} ({sess['bytes']/1e6:.0f} MB)")
    pct, free_gb = _disk_usage_pct()
    if _disk_alert["active"] and pct < _DISK_CLEAR_PCT:
        _disk_alert = {"active": False, "pct": round(pct, 1), "free_gb": round(free_gb, 2)}
        await broadcast({"type": "disk_alert", **_disk_alert})
    return {"ok": True, "deleted": deleted, "pct": round(pct, 1), "free_gb": round(free_gb, 2)}


@app.post("/api/totals/reset")
def totals_reset():
    """LEGITIMATE lifetime-totals reset (2.13.32): archive the current totals
    to machineTotals_<date>_<time>.json, then zero the guard baseline, the
    statics, and machineTotals.json together -- the monotonic guard accepts
    it because its own reference is updated. No server stop needed (the old
    procedure). The dated archive keeps the odometer history reviewable."""
    global _machine_totals
    import time as _t
    old = dict(_machine_totals)
    live = _totals_backend_get()
    if live:
        for k in MACHINE_TOTALS_KEYS:
            old[k] = max(old.get(k, 0.0), live[k])
    stamp = _t.strftime("%Y-%m-%d_%H%M%S")
    arch = CFG_DIR / f"machineTotals_{stamp}.json"
    try:
        body = {k: old[k] for k in MACHINE_TOTALS_KEYS}
        body["note"] = f"lifetime totals archived at reset {stamp}"
        arch.write_text(json.dumps(body, indent=2))
    except Exception as e:
        return {"ok": False, "error": f"archive failed -- totals NOT reset: {e}"}
    _machine_totals = {k: 0.0 for k in MACHINE_TOTALS_KEYS}
    _totals_backend_set(_machine_totals)
    try:
        _save_machine_totals()
    except Exception as e:
        print(f"[TOTALS] save after reset failed: {e}")
    print(f"[TOTALS] RESET -- archived {arch.name}: " +
          ", ".join(f"{k}={old[k]:.3f}" for k in MACHINE_TOTALS_KEYS))
    return {"ok": True, "archived": arch.name,
            "previous": {k: old[k] for k in MACHINE_TOTALS_KEYS}}
...
def launch_log_viewer(body: dict = None):
    ...
    if scales or names or charted or colors:
        try:
    ...
    if body.get("bundle_only"):
        # LOCAL-VIEWER flow (2.13.34): the clicking browser invokes its own
        # machine's mccviewer: protocol handler (viewer_local_launch.py),
        # which downloads the log + this bundle and opens log_viewer.py on
        # THAT computer. Here we only prepare the bundle -- no rig launch.
    ...
    try:
        ...
        if sys.platform == "win32":
            ...
            # RIG DISPLAY ATTACH (2.13.33): the systemd service has no display
            # env, so the Tk viewer died with "no $DISPLAY". The Pi's kiosk
            # session runs Xwayland on :0 owned by piconnect -- borrow its
            # auth cookie (sudo cp to an mvr-owned copy) and the viewer opens
            # on the RIG'S TOUCHSCREEN. (The viewer always appears on the
            # server's own display, never on a remote browser's machine.)
            env = dict(os.environ)
            if not env.get("DISPLAY"):
                try:
                    xa = "/tmp/.mcc_viewer_xauth"
                    subprocess.run(["sudo", "-n", "cp", "/home/piconnect/.Xauthority", xa],
                                   check=True, timeout=5)
                    subprocess.run(["sudo", "-n", "chown", str(os.getuid()), xa],
                                   check=True, timeout=5)
                    env["DISPLAY"] = ":0"
                    env["XAUTHORITY"] = xa
                    kwargs["env"] = env
                    print("[MCC-Hub] Viewer display: attached to the kiosk session (:0)")
                except Exception as e:
                    print(f"[MCC-Hub] Viewer display attach failed ({e}) -- "
                          "no display for the Tk viewer; it will exit immediately")
