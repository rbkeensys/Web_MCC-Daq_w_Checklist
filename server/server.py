...
def should_recompile_cpp_expressions():
    ...
    _vers = list(_compiled.glob("expressions_v*.dll")) if _compiled.exists() else []
    try:
        dll_file = max(_vers, key=lambda p: int(p.stem.split('_v')[1])) if _vers else Path("compiled/expressions.dll")
    except Exception:
    ...
    for ref in (expr_json, PID_PATH, SCALES_PATH,
                Path("compiled/expressions.dll"), Path("compiled/expr_metadata.json")):
        try:
...
try:
    _vp = list(Path("compiled").glob("expressions_v*.dll"))
    if _vp:
...
if should_recompile_cpp_expressions():
    if compile_cpp_expressions():
        ...
        _cdir = Path("compiled")
        _dlls = list(_cdir.glob("expressions_v*.dll")) if _cdir.exists() else []
        if _dlls:
